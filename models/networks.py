...
def define_G(netG,init_type='normal', init_gain=0.02, gpu_ids=[],channels=64):
    net = None
    init_weight = True

    if netG == 'SADCNet':
        net = ShadowRemoval(channels=channels)
    else:
...
class ConsistencyLoss(nn.Module):
    ...
    def forward(self, activation):
        ...
        for key in activation.keys():
            loss += self.kl_div(activation[key][0].log_softmax(dim=-1), activation[key][1].softmax(dim=-1)) / len(activation)
